...
- **`recommendation-service`, `notification-service`, `analytics-service` are static/stubbed today** despite their names implying real logic - no database, no RabbitMQ, no calls to other services. `notification-service` only logs; `analytics-service` returns a hardcoded map; `recommendation-service` returns hardcoded lists. Not partially-implemented-and-broken, just genuinely not built yet.
